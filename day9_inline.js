...
const nextField = (marblePos, field, marbleValue) =>
  field.splice(marblePos, 0, marbleValue)
...
const nextState = (state, [marbleValue, player]) => {
  const { field, marblePos, playerValuations } = state

  if (marbleValue % 23 === 0) {
    const nextMarblePos = nextMarblePosInValuationRound(marblePos, field.length)
    playerValuations[player] += marbleValue + field[nextMarblePos]
    state.marblePos = nextMarblePos === field.length - 1 ? 0 : nextMarblePos
    nextFieldInValuationRound(nextMarblePos, field)
  } else {
    state.marblePos = nextMarblePos(marblePos, field.length)
    nextField(state.marblePos, field, marbleValue)
  }
